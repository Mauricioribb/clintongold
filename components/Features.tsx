...
const FEATURE_LIST = [
  ...
  {
    icon: <EyeOff size={40} className="text-white" />,
    ...
    <section className="py-12 md:py-24 bg-white overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="text-center mb-8 md:mb-16">
          ...
          <h3 className="text-3xl md:text-4xl font-bold uppercase tracking-tighter text-black">Por que escolher a Clinton Gold?</h3>
        </div>

        <div className="relative flex items-center justify-center min-h-[450px]">
            <button
              onClick={prevSlide}
              className="absolute left-0 md:left-4 z-30 p-3 rounded-full bg-black/10 hover:bg-gold text-black hover:text-white transition-all duration-300 hidden md:block"
            >
              <ChevronLeft size={24} />
            </button>
            ...
            <button 
              onClick={nextSlide}
              className="absolute right-0 md:right-4 z-30 p-3 rounded-full bg-black/10 hover:bg-gold text-black hover:text-white transition-all duration-300 hidden md:block"
            >
            ...
            {FEATURE_LIST.map((feature, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`transition-all duration-700 ease-in-out cursor-pointer flex flex-col items-center text-center p-10 md:p-14 bg-black text-white rounded-[10px] shadow-2xl w-full max-w-[320px] md:max-w-[400px] border border-white/5 ${positionClass}`}
                >
                  <div className="mb-8 text-white transform transition-transform group-hover:scale-110 duration-500">
                    {feature.icon}
                  </div>
                  <h3 className="text-2xl font-black mb-4 uppercase tracking-tighter text-white">
                    {feature.title}
                  </h3>
          ...
          {FEATURE_LIST.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-500 ${
                i === activeIndex ? 'w-10 bg-gold' : 'w-2 bg-black/20 hover:bg-black/40'
              }`}
